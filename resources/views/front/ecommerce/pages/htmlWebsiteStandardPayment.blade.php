...
<form style=" padding:0px;margin:0px;" name="frmOrderAutoSubmit" method="post" >
	...
	<input type="hidden" name="business" value="firozsabuz@gmail.com">
	...
	@foreach(Cart::content() as $content)
		...
		<input type="hidden" name="quantity_{{ $i }}" value="{{ $content->qty }}" />
		<input type="hidden" name="item_name_{{ $i }}" value="{{ $content->name }}">
		<input type="hidden" name="amount_{{ $i }}" value="{{ $content->price }}">
	...
	$customer = App\Shipping::where('customer_id',$customer_id)->first();
	?>
	<input type="hidden" name="rm" value="2" />
	<input TYPE="hidden" name="address_override" value="0">
	<input type="hidden" name="first_name" value="{{ $customer->company_name }}">
	...
	<input type="hidden" name="state" value="{{ $customer->country }}">
